...
import re
import os
from config import EXCEL_PATH, GBF_CDN_URL
...
def check_evo(workSheet, excelListPath, ID_set):
    ...
    for cell in workSheet2[evo_index2][2:]:
        ...
        id_info = ID_set.get(characterID)
        if not id_info:
            continue
        if id_info[1]:
            continue
        workSheet[extra_index][id_info[0]].value = '03'
...
def merge_anime_excel(cfg, args):
    ...
    # 合并后刷新一次ID索引，确保新追加行也能参与终突标记。
    ID_set = {}
    for row in range(1, workSheet.max_row):
        character_id = workSheet[ID_index][row].value
        if not isinstance(character_id, int) or character_id <= 0:
            continue
        ID_set[character_id] = (row, workSheet[extra_index][row].value)
...
def check_anime_file(cfg, args):
    ...
    unusedIndex = toIndex.get('unused')
    if unusedIndex is None:
        print('警告：角色动画.xlsx 未找到 unused 列，将跳过冗余动画ID记录。')
    ...
    def download_js(head, charID, row, col):
        ...
        if not workSheet[col][row].value:
            ...
            if workSheet[unusedIndex][row].value:
                ...
                if head+charID not in unusedList:
    ...
    def download_img(charID):
        if mode == 0:
            ...
            fileName = charID+'.png'
        link = GBF_CDN_URL + '/assets/img/sp/cjs/' + charID + '.png'
        downloader.download_multi_copies(
            link, [pathImg+fileName, imgUploadPath+fileName])
        return True

    def normalize_anime_version(value):
        token = str(value).strip()
        if token.isdigit() and len(token) == 1:
            return '0' + token
        return token

    def parse_extra_versions(cell_value):
        if cell_value is None:
            return []
        if isinstance(cell_value, str):
            raw_list = cell_value.split(';')
        elif isinstance(cell_value, (int, float)):
            if isinstance(cell_value, float) and not cell_value.is_integer():
                raw_list = [str(cell_value)]
            else:
                raw_list = [str(int(cell_value))]
        else:
            raw_list = [str(cell_value)]

        result = []
        for raw_value in raw_list:
            version = normalize_anime_version(raw_value)
            if version == '':
                continue
    ...
    for row in range(2, maxRow):
        ...
        if tupleNpcIndex[row].value:
            if tupleExtraIndex[row].value:
                typeList = parse_extra_versions(tupleExtraIndex[row].value)
