...
IMG_CONFIG = [
    {'name': '抬头立绘', 'filename_prefix': 'profile', 'sub_url': 'profile', 'fallback_sub_url': 'zoom', 'suffix': 'png', 'has_extra': True, 'skip_index': [], 'multi_element': True, 'skin': False, 'multi': True, 'multi_allow_index': ['01']},
    ...
    for npc_id, npc_info in tabx:
        ...
        for config in IMG_CONFIG:
            ...
            for index in config_index_list:
                ...
                # 立绘可能存在性别差分后缀（_0 / _1）
                if config["filename_prefix"] in ['profile', 'zoom']:
        ...
        if len(work_list) > 0:
            ...
            for work_info in work_list:
                if save_to_new:
                    downloader.download_multi_copies(work_info['url'], [work_info['path'], work_info['new']], fallback_url=work_info.get('fallback_url'))
                else:
                    downloader.download_multi_copies(work_info['url'], [work_info['path']], fallback_url=work_info.get('fallback_url'))
