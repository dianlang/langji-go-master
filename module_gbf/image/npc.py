...
IMG_CONFIG = [
    {'name': '立绘', 'filename_prefix': 'profile', 'sub_url': 'profile', 'fallback_sub_url': 'zoom', 'suffix': 'png', 'has_extra': True, 'skip_index': [], 'multi_element': True, 'skin': False, 'multi': True, 'multi_allow_index': ['01']},
...
def npc(cfg):
    ...
    skip_log_f = open(SKIP_LOG, 'a', encoding='utf-8')

    def normalize_extra_index_list(raw_value):
        if type(raw_value) == list:
            raw_list = raw_value
        elif raw_value is None:
            raw_list = []
        else:
            raw_list = str(raw_value).split(';')

        result = []
        invalid_tokens = {'', 'none', 'null', '-', '--', '无', '暂无'}
        filtered_raw = []
        for raw_index in raw_list:
            index = str(raw_index).strip()
            if index.lower() in invalid_tokens:
                filtered_raw.append(index)
                continue
            if index.isdigit() and len(index) == 1:
                index = '0' + index
            if index not in result:
                result.append(index)
        return result, filtered_raw
    ...
    for npc_id, npc_info in tabx:
        ...
        elif npc_info['max_evo'] == 6:
            index_list.append('03')
        raw_extra_img = npc_info.get('extra_img', [])
        extra_index_list, filtered_tokens = normalize_extra_index_list(raw_extra_img)
        if len(filtered_tokens) > 0:
            skip_log_f.write('[extra_img filtered] ID=%s name=%s raw=%s filtered=%s\n' % (
                npc_id,
                npc_info.get('name_chs', ''),
                str(raw_extra_img),
                ';'.join(filtered_tokens)
            ))

        raw_gender_img = npc_info.get('gender_img', [])
        gender_index_list, gender_filtered_tokens = normalize_extra_index_list(raw_gender_img)
        if len(gender_filtered_tokens) > 0:
            skip_log_f.write('[gender_img filtered] ID=%s name=%s raw=%s filtered=%s\n' % (
                npc_id,
                npc_info.get('name_chs', ''),
                str(raw_gender_img),
                ';'.join(gender_filtered_tokens)
            ))
        ...
        for config in IMG_CONFIG:
            ...
            # 仅当六突角色时，额外尝试04立绘，避免其它资源类型产生大量404。
            if npc_info['max_evo'] == 6 and config["filename_prefix"] == 'profile' and '04' not in config_index_list:
                config_index_list.append('04')
            if config['has_extra']:
            ...
            for index in config_index_list:
                ...
                # 立绘可能存在性别差分后缀（_0 / _1）
                if config["filename_prefix"] in ['profile', 'zoom'] and index in gender_index_list:
                    img_name_list.extend(['%s_0' % img_base_name, '%s_1' % img_base_name])
                ...
                for img_name in img_name_list:
                    ...
                    fallback_url = None
                    if "fallback_sub_url" in config:
                        fallback_url = '%s/%s/%s.%s' % (npc_base_url, config["fallback_sub_url"], img_name, config["suffix"])

                    # 若备用URL曾404并进入skip，也一并跳过主URL，减少重复无效请求。
                    if img_url in skip_list or (fallback_url and fallback_url in skip_list):
                        continue
        ...
        if len(work_list) > 0:
            ...
            for work_info in work_list:
                if save_to_new:
                    downloader.download_multi_copies(work_info['url'], [work_info['path'], work_info['new']], log_handle=skip_log_f, fallback_url=work_info.get('fallback_url'))
                else:
                    downloader.download_multi_copies(work_info['url'], [work_info['path']], log_handle=skip_log_f, fallback_url=work_info.get('fallback_url'))
